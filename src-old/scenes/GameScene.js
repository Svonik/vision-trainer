...
import { COLORS, GAME, SPEEDS, PLATFORM_KEYBOARD_SPEED } from '../config/constants.js';
import { createGameSettings, createSessionResult } from '../modules/gameState.js';
import { createSafetyTimer } from '../modules/safetyTimer.js';
import { addSession } from '../modules/storage.js';
...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.safetyTimer = createSafetyTimer({
      onWarning: () => this.showWarning(t('safety.breakWarning')),
      onBreak: () => this.onBreakTime(),
    });
  ...
  showWarning(message) {
    const cx = this.cameras.main.centerX;
    const warning = this.add.text(cx, 50, message, {
      fontSize: '16px', color: '#FFAA00', fontFamily: 'Arial, sans-serif',
    }).setOrigin(0.5);
    this.tweens.add({
      targets: warning, alpha: 0, delay: 3000, duration: 1000,
  ...
  onBreakTime() {
    ...
    const bg = this.add.rectangle(cx, cy, 400, 250, COLORS.BLACK, 0.9)
      .setStrokeStyle(2, COLORS.WHITE);
    elements.push(bg);

    elements.push(this.add.text(cx, cy - 70, t('safety.breakTime'), {
      fontSize: '28px', color: '#FFAA00', fontFamily: 'Arial, sans-serif',
    }).setOrigin(0.5));

    elements.push(this.add.text(cx, cy - 20, t('safety.breakMessage'), {
      fontSize: '14px', color: COLORS.GRAY_HEX, fontFamily: 'Arial, sans-serif',
      wordWrap: { width: 350 }, align: 'center',
    }).setOrigin(0.5));

    const finishBtn = this.add.rectangle(cx, cy + 40, 200, 40, COLORS.GRAY, 0.2)
      .setStrokeStyle(1, COLORS.GRAY).setInteractive({ useHandCursor: true });
    elements.push(finishBtn);
    elements.push(this.add.text(cx, cy + 40, t('safety.finish'), {
      fontSize: '16px', color: COLORS.WHITE_HEX, fontFamily: 'Arial, sans-serif',
    }).setOrigin(0.5));
    finishBtn.on('pointerup', () => this.endGame());

    if (this.safetyTimer.canExtend()) {
      const extBtn = this.add.rectangle(cx, cy + 90, 240, 40, COLORS.GRAY, 0.2)
        .setStrokeStyle(1, COLORS.GRAY).setInteractive({ useHandCursor: true });
      elements.push(extBtn);
      elements.push(this.add.text(cx, cy + 90, t('safety.extend'), {
        fontSize: '16px', color: COLORS.WHITE_HEX, fontFamily: 'Arial, sans-serif',
      }).setOrigin(0.5));
      extBtn.on('pointerup', () => {
  ...
  endGame() {
    ...
    addSession(result);
    this.scene.start('StatsScene', { result, settings: this.settings });
  }
}

...
export function QuantitativeSuppressionStep({
    glassesType,
    onComplete,
}: QuantitativeSuppressionStepProps) {
    const [state, setState] = useState<SuppressionTestState>(() =>
        advanceContrast(createSuppressionTest()),
    );
        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-6 space-y-6 bg-[var(--bg)]">
            ...
            <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                <button
                    ...
                    className="rounded-3xl border-2 border-[var(--border)] bg-[var(--surface)] hover:border-[var(--accent)] p-4 btn-press transition-[transform,box-shadow,border-color]"
                >
                ...
                <button
                    type="button"
                    onClick={handleSeen}
                    className="rounded-3xl border-2 border-[var(--cta)] bg-[var(--surface)] shadow-[0_0_25px_rgba(255,159,67,0.3)] p-4 btn-press transition-[transform,box-shadow,border-color]"
                >

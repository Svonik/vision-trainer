...
export default class AsteroidGameScene extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.image(
            'asteroid-medium',
            'assets/sprites/asteroid-medium.png',
        );
    ...
    startGameplay() {
        ...
        this.controlsHint = this.add
            .text(
                ccx,
                fy + fh - 20,
                '← → вращение  ↑ тяга  Пробел/клик огонь',
                {
                    fontSize: '12px',
                    color: COLORS.GRAY_HEX,
                    fontFamily: 'Arial, sans-serif',
        ...
        this.speedMultiplier =
            ASTEROID_SPEED_MULTIPLIERS[this.settings.speed] || 1.0;
    ...
    spawnAsteroid(size, x, y) {
        ...
        if (ax === null || ay === null) {
            ...
            do {
                ...
                if (edge === 0) {
                    ax = fx + Math.random() * fw;
                    ay = fy + def.radius;
                } else if (edge === 1) {
                    ax = fx + fw - def.radius;
                    ay = fy + Math.random() * fh;
                } else if (edge === 2) {
                    ax = fx + Math.random() * fw;
                    ay = fy + fh - def.radius;
                } else {
        ...
        const rotSpeed =
            ...
            ASTEROID_ROT_SPEED *
            (0.5 + Math.random()) *
            (size === 'small' ? 2 : size === 'medium' ? 1.4 : 1);
        ...
        if (this.textures.exists(spriteKey)) {
            ...
            const astScale =
                (def.radius * 2) / Math.max(sprite.width, sprite.height);
    ...
    update(time, delta) {
        ...
        this.shipVX *= SHIP_FRICTION ** (delta / 16.667);
        this.shipVY *= SHIP_FRICTION ** (delta / 16.667);
        ...
        if (this.bulletsGraphics) {
            ...
            for (const b of this.bullets) {
                if (!b.sprite && b.age < BULLET_LIFETIME_MS) {
                    this.bulletsGraphics.fillStyle(
                        this.platformColor,
                        this.platformAlpha * 0.3,
                    );
        ...
        const hasGraphicsAsteroids = this.asteroids.some(
            (a) => a.active && !a.sprite,
        );
    ...
    renderAsteroidsFallback() {
        ...
        for (const a of this.asteroids) {
            ...
            for (let i = 0; i <= numPoints; i++) {
                ...
                const px =
                    a.x + Math.cos(theta) * a.radius * a.scaleX * wobble * 1.5;
                const py =
                    a.y + Math.sin(theta) * a.radius * a.scaleY * wobble * 1.5;
    ...
    hitAsteroid(asteroid) {
        ...
        this.asteroidsDestroyed++;
        if (this.hud)
            this.hud.scoreText.setText(`★ ${this.asteroidsDestroyed}`);
    ...
    updateFellowEyeAlpha(alpha) {
        // Update ship (fellow eye) alpha — asteroids (amblyopic eye) stay at 1.0
    ...
    endGame(won) {
        ...
        const result = {
            ...
            hit_rate:
                this.totalAsteroids > 0
                    ? Math.round(
                          (this.asteroidsDestroyed / this.totalAsteroids) * 100,
                      ) / 100
                    : 0,

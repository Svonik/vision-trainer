 ...
 * fellow/strong-eye object; the balloon body is fixed at alpha 1.0
...
export function resolveChannelColors(
    ...
    return { crosshairColor: fellowColor, balloonColor: amblyopicColor };
}


export default class BalloonPopGameScene extends DichopticScene {
    constructor() {
    ...
    startGameplay() {
        ...
        this.balloonLifespan =
            LIFESPAN_MS[this.settings.speed] || LIFESPAN_MS.normal;
    ...
    popBalloon(index) {
        ...
        this.popped++;
        if (this.hud)
            this.hud.scoreText.setText(`★ ${this.popped}/${WIN_COUNT}`);
    ...
    resetForNextLevel() {
        ...
        if (this.hud)
            this.hud.scoreText.setText(`★ ${this.popped}/${WIN_COUNT}`);
        this.balloonLifespan = Math.max(
            500,
            Math.round(this.balloonLifespan * 0.85),
        );
    }

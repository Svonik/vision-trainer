...
export default class BreakoutGameScene extends Phaser.Scene {
    ...
    startGameplay() {
        ...
        this.launchHint = this.add
            .text(ccx, ccy + 60, t('breakout.launchHint'), {
                fontSize: '14px',
    ...
    _spawnBricks() {
        ...
        for (let row = 0; row < rows; row++) {
            for (let col = 0; col < cols; col++) {
                ...
                const brickVisual = GameVisuals.glowRect(
                    this,
                    bx,
                    by,
                    brickW - 6,
    ...
    update(_time, delta) {
        ...
        if (!this.ballLaunched && this.ball) {
            ...
            this.ball.y =
                this.platform.y - this.platformH / 2 - this.ballRadius - 2;
        }
        ...
        if (this.ball && this.ballLaunched) {
            ...
            if (this.ball.y - r <= this.field.y) {
                this.ball.body.setVelocityY(
                    Math.abs(this.ball.body.velocity.y),
                );
        ...
        if (this.extraBalls) {
            ...
            for (let i = this.extraBalls.length - 1; i >= 0; i--) {
                ...
                if (eb.y - r <= this.field.y) {
                    ...
                    if (
                        now - this.lastWallBounceTime >
                        WALL_BOUNCE_COOLDOWN_MS
                    ) {
                        SynthSounds.tick();
                        this.lastWallBounceTime = now;
                    }
        ...
        if (this.powerups) {
            for (let i = this.powerups.length - 1; i >= 0; i--) {
                ...
                if (
                    ...
                    Math.abs(pu.container.x - this.platform.x) <
                        this.platformW / 2
                ) {
        ...
        switch (type) {
            ...
            case 'multi': {
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    const extraBall = this.add.circle(
                        this.ball.x,
                        this.ball.y,
                        this.ballRadius,
                        this.ballColor,
    ...
    endGame(won) {
        ...
        const result = {
            ...
            hit_rate:
                this.totalBricks > 0
                    ? Math.round(
                          (this.bricksDestroyed / this.totalBricks) * 100,
                      ) / 100
                    : 0,

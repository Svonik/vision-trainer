...
export default class CatchMonstersGameScene extends Phaser.Scene {
    ...
    startGameplay() {
        ...
        this.crosshair = {
            h: this.add
                .rectangle(ccx, ccy, CROSSHAIR_SIZE * 2, 2, this.platformColor)
                .setAlpha(this.platformAlpha),
            v: this.add
                .rectangle(ccx, ccy, 2, CROSSHAIR_SIZE * 2, this.platformColor)
                .setAlpha(this.platformAlpha),
            ring: this.add
                .circle(ccx, ccy, CROSSHAIR_SIZE / 2, this.platformColor, 0)
    ...
    spawnMonster() {
        ...
        const tierMultiplier =
            1 +
            Math.floor(this.monstersCaught / CATCHES_PER_TIER) *
                SPEED_BOOST_PER_TIER;
        const effectiveBaseSpeed = this.baseSpeed * tierMultiplier;

        const circle = GameVisuals.glowCircle(
            this,
            mx,
            my,
            radius,
    ...
    getNextDirChange() {
        return (
            DIRECTION_CHANGE_MIN_MS +
            Math.random() * (DIRECTION_CHANGE_MAX_MS - DIRECTION_CHANGE_MIN_MS)
        );
    }
    ...
    catchMonster(index) {
        ...
        if (this.hud)
            this.hud.scoreText.setText(
                `★ ${this.monstersCaught}/${WIN_CATCHES}`,
            );
        ...
        if (newTier > this.speedTier) {
            ...
            const _tierMultiplier = 1 + this.speedTier * SPEED_BOOST_PER_TIER;
        ...
        this.time.delayedCall(500, () => {
            if (
                !this.isPaused &&
                this.monsters.length < this.activeMonsterCount
            ) {
                this.spawnMonster();
            }
        });
    ...
    update(_time, delta) {
        ...
        for (const m of this.monsters) {
            ...
            if (m.dirChangeTimer <= 0) {
                const tierMultiplier =
                    1 + this.speedTier * SPEED_BOOST_PER_TIER;
                const effectiveBaseSpeed = this.baseSpeed * tierMultiplier;
                const angle = Math.random() * Math.PI * 2;
                const speed = effectiveBaseSpeed * (0.8 + Math.random() * 0.4);
            ...
            else if (m.circle.x > fx + fw + m.radius)
                m.circle.x = fx - m.radius;
            if (m.circle.y < fy - m.radius) m.circle.y = fy + fh + m.radius;
            else if (m.circle.y > fy + fh + m.radius)
                m.circle.y = fy - m.radius;
    ...
    endGame(won) {
        ...
        const result = {
            ...
            hit_rate:
                this.totalMonsters > 0
                    ? Math.round(
                          (this.monstersCaught / this.totalMonsters) * 100,
                      ) / 100
                    : 0,

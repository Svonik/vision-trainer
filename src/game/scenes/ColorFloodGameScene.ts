...
export default class ColorFloodGameScene extends Phaser.Scene {
    ...
    renderGrid() {
        ...
        const cellSize = Math.floor(
            Math.min(availW / this.gridSize, availH / this.gridSize),
        );
    ...
    renderButtons() {
        ...
        for (let i = 0; i < TYPE_COUNT; i++) {
            ...
            const hit = this.add
                .rectangle(bx, by, btnSize, btnSize, 0x000000, 0)
                .setInteractive({ useHandCursor: true });
    ...
    animateFloodWave(prevSize) {
        ...
        for (const key of this.floodRegion) {
            if (idx >= prevSize) {
                const [r, c] = key.split(',').map(Number);
                const obj = this.cellObjects.find(
                    (o) => o.r === r && o.c === c,
                );
                if (obj) {
                    GameVFX.flash(
    ...
    endGame(won) {
        ...
        const result = {
            ...
            hit_rate:
                totalCells > 0
                    ? Math.round((this.floodRegion.size / totalCells) * 100) /
                      100
                    : 0,

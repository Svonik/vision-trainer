...
export default class Game2048Scene extends Phaser.Scene {
    ...
    startGameplay() {
        ...
        const gridW =
            this.cellSize * GRID_SIZE + TILE_SPACING * (GRID_SIZE + 1);
        const gridH = gridW;
    ...
    cellCenter(row: number, col: number): { x: number; y: number } {
        return {
            x:
                this.gridX +
                TILE_SPACING +
                col * (this.cellSize + TILE_SPACING) +
                this.cellSize / 2,
            y:
                this.gridY +
                TILE_SPACING +
                row * (this.cellSize + TILE_SPACING) +
                this.cellSize / 2,
        };
    }
    ...
    createTileSprite(row: number, col: number, tileValue: number): TileSprite {
        ...
        const fontSize =
            tileValue >= 10 ? '16px' : tileValue >= 7 ? '20px' : '26px';
        ...
        this.tweens.add({
            ...
            onComplete: () => {
                ...
                if (isMerge) {
                    ...
                    this.tileSprites[toRow][toCol] = this.createTileSprite(
                        toRow,
                        toCol,
                        newValue,
                    );

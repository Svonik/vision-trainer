...
export default class GameScene extends Phaser.Scene {
    ...
    update(_time, delta) {
        ...
        if (this.safetyTimer && this.hud) {
            GameVisuals.updateHUD(
                ...
                this.level,
                this.safetyTimer.getElapsedMs(),
                `★ ${this.caught}/${GAME.TARGET_CATCHES}`,
            );
        }
    }
    ...
    spawnTarget() {
        ...
        const targetVisual = GameVisuals.glowCircle(
            this,
            x,
            this.field.y - objDiameter,
            objDiameter / 2,
    ...
    onCatch(platform, target) {
        ...
        const flash = this.add
            .circle(target.x, target.y, 20, COLORS.WHITE)
            .setAlpha(0.8);
        this.tweens.add({
            targets: flash,
            alpha: 0,

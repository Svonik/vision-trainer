...
export default class InvadersGameScene extends Phaser.Scene {
    ...
    startGameplay() {
        ...
        this.alienStepInterval =
            ALIEN_STEP_INTERVALS[this.settings.speed] || 1000;
        ...
        if (this.textures.exists('ship')) {
            ...
            this.physics.add.existing(this.ship, false);
            this.ship.body.setImmovable(false);
            this.shipVisual = GameVisuals.glowRect(
                this,
                ccx,
                shipY,
                shipW,
        ...
        for (let row = 0; row < ALIEN_ROWS; row++) {
            for (let col = 0; col < ALIEN_COLS; col++) {
                ...
                const spriteKey =
                    ALIEN_SPRITE_KEYS[row % ALIEN_SPRITE_KEYS.length];
                ...
                if (this.textures.exists(spriteKey)) {
                    ...
                    const alien = this.add.rectangle(
                        ax,
                        ay,
                        ALIEN_W,
                        ALIEN_H,
                    ...
                    const alienVisual = GameVisuals.glowRect(
                        this,
                        ax,
                        ay,
                        ALIEN_W,
                    ...
                    const notchL = this.add
                        .rectangle(ax - 8, ay - 4, 6, 6, this.alienColor)
                        .setAlpha(this.alienAlpha * 0.8);
                    const notchR = this.add
                        .rectangle(ax + 8, ay - 4, 6, 6, this.alienColor)
                        .setAlpha(this.alienAlpha * 0.8);
        ...
        this.touchFire = TouchControls.createActionButton(
            this,
            this.field,
            '🔥',
        );
    ...
    update(time, delta) {
        ...
        for (const alienEntry of this.aliens) {
            if (!alienEntry.body || !alienEntry.body.active) continue;
            if (
                alienEntry.body.y + ALIEN_H / 2 >=
                this.field.y + this.field.h - 30
            ) {
                this.endGame(false);
                return;
            }
        ...
        if (this.safetyTimer && this.hud) {
            GameVisuals.updateHUD(
                ...
                `★ ${this.enemiesDestroyed}/${TOTAL_ALIENS}`,
                this.lives,
            );
        }
    }
    ...
    marchAliens() {
        ...
        if (reverseAndStep) {
            ...
            for (const a of liveAliens) {
                ...
                if (a.notchL) {
                    a.notchL.x = a.body.x - 8;
                    a.notchL.y = a.body.y - 4;
                }
                if (a.notchR) {
                    a.notchR.x = a.body.x + 8;
                    a.notchR.y = a.body.y - 4;
                }
                if (a.alienVisual) {
                    a.alienVisual.x = a.body.x;
                    a.alienVisual.y = a.body.y;
                }
            }
        ...
        this.alienStepInterval = Math.max(
            150,
            baseInterval * Math.max(0.3, fraction),
        );
    }
    ...
    fireEnemyBullet() {
        ...
        let bullet;
        const laserKey = this.textures.exists('laser-red')
            ? 'laser-red'
            : this.textures.exists('laser')
              ? 'laser'
              : null;
        if (laserKey) {
            bullet = this.add.image(
    ...
    destroyAlien(alienEntry, index) {
        ...
        const flash = this.add
            .rectangle(alien.x, alien.y, ALIEN_W, ALIEN_H, COLORS.WHITE)
            .setAlpha(0.9);
        this.tweens.add({
            targets: flash,
            alpha: 0,
        ...
        if (alienEntry.notchL) {
            alienEntry.notchL.destroy();
            alienEntry.notchL = null;
        }
        if (alienEntry.notchR) {
            alienEntry.notchR.destroy();
            alienEntry.notchR = null;
        }
        if (alienEntry.alienVisual) {
            alienEntry.alienVisual.destroy();
            alienEntry.alienVisual = null;
        }
        alien.destroy();
        this.aliens[index] = {
            body: { active: false },
    ...
    loseLife() {
        ...
        let blinkCount = 0;
        const totalBlinks = Math.floor(
            INVINCIBILITY_DURATION_MS / BLINK_INTERVAL_MS,
        );
    ...
    resetForNextLevel() {
        ...
        for (const alienEntry of this.aliens) {
            ...
            if (alienEntry.body?.active) alienEntry.body.destroy();
        }
    ...
    endGame(won) {
        ...
        const result = {
            ...
            hit_rate:
                TOTAL_ALIENS > 0
                    ? Math.round((this.enemiesDestroyed / TOTAL_ALIENS) * 100) /
                      100
                    : 0,

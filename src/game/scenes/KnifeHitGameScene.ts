...
export default class KnifeHitGameScene extends Phaser.Scene {
  ...
  startGameplay() {
    ...
    this.knifeColor = isKnifeLeft ? eyeColors.leftColor : eyeColors.rightColor;
    this.stuckColor = isKnifeLeft ? eyeColors.rightColor : eyeColors.leftColor;
    this.knifeAlpha = (isKnifeLeft ? this.settings.contrastLeft : this.settings.contrastRight) / 100;
    this.stuckAlpha = (isKnifeLeft ? this.settings.contrastRight : this.settings.contrastLeft) / 100;
    ...
    this.rotationVariation = ROTATION_VARIATION[speed] || ROTATION_VARIATION.normal;
    ...
    // ── Input ──
    this.input.on('pointerup', () => { if (!this.isPaused) this.throwKnife(); });
  ...
  drawTarget() {
    ...
    this.targetGfx.fillCircle(this.targetX, this.targetY, TARGET_RADIUS + 10);
    // Main circle
    ...
    this.targetGfx.strokeCircle(this.targetX, this.targetY, TARGET_RADIUS * 0.7);
    this.targetGfx.strokeCircle(this.targetX, this.targetY, TARGET_RADIUS * 0.4);
    // Center dot
  ...
  drawWaitingKnife() {
    ...
    this.waitingKnife.fillRect(x - KNIFE_WIDTH / 2, y - KNIFE_LENGTH, KNIFE_WIDTH, KNIFE_LENGTH);
    // Handle (slightly wider, below)
    this.waitingKnife.fillStyle(this.knifeColor, this.knifeAlpha * 0.6);
    this.waitingKnife.fillRect(x - KNIFE_WIDTH, y, KNIFE_WIDTH * 2, KNIFE_LENGTH * 0.5);
    // Glow
    this.waitingKnife.fillStyle(this.knifeColor, this.knifeAlpha * 0.08);
    this.waitingKnife.fillCircle(x, y - KNIFE_LENGTH / 2, KNIFE_LENGTH * 0.8);
  }
  ...
  drawStuckKnife(angleDeg) {
    ...
    const baseX = this.targetX + Math.cos(rad) * (TARGET_RADIUS + KNIFE_LENGTH);
    const baseY = this.targetY + Math.sin(rad) * (TARGET_RADIUS + KNIFE_LENGTH);
  ...
  drawLives() {
    ...
    for (let i = 0; i < this.lives; i++) {
      const icon = this.add.text(startX - i * 16, y, '\u2764', {
  ...
  changeRotationSpeed() {
    ...
    const variation = Phaser.Math.FloatBetween(-this.rotationVariation, this.rotationVariation);
  ...
  placeInitialKnives(count) {
    for (let i = 0; i < count; i++) {
      ...
      this.stuckKnives = [...this.stuckKnives, { angle: normalised, graphics: gfx }];
    }
  }
  ...
  onKnifeArrived() {
    ...
    const collision = this.stuckKnives.some(sk => {
      const diff = Math.abs(Phaser.Math.Angle.ShortestBetween(impactAngle, sk.angle));
  ...
  onSuccessfulStick(angleDeg) {
    ...
    this.stuckKnives = [...this.stuckKnives, { angle: angleDeg, graphics: gfx }];
    this.knivesStuck++;
  ...
  onCollision() {
    ...
    fallKnife.fillRect(x - KNIFE_WIDTH / 2, this.targetY + TARGET_RADIUS - KNIFE_LENGTH, KNIFE_WIDTH, KNIFE_LENGTH);
    ...
    this.tweens.add({
      ...
      onUpdate: () => {
        ...
        fallKnife.fillRect(x - KNIFE_WIDTH / 2, this.targetY + TARGET_RADIUS + fallObj.y, KNIFE_WIDTH, KNIFE_LENGTH);
      },
      onComplete: () => {
        fallKnife.destroy();
        this.resetWaitingKnife();
  ...
  redrawStuckKnives() {
    this.stuckKnives.forEach(sk => {
      ...
      sk.graphics.lineStyle(KNIFE_WIDTH, this.stuckColor, this.stuckAlpha);
  ...
  update(time, delta) {
    ...
    if (this.hud) {
      ...
      GameVisuals.updateHUD(this.hud, this.level, this.safetyTimer.getElapsedMs(), scoreStr);
    }
  }
  ...
  endGame(won) {
    ...
    const result = {
      ...
      hit_rate: totalAttempts > 0
        ? Math.round((this.contrastState.totalHits / totalAttempts) * 100) / 100
        : 0,

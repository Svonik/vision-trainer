...
const GEM_SHAPES = ['circle', 'square', 'triangle', 'diamond', 'star', 'hexagon'] as const;
...
const RED_GEMS: GemShape[] = ['circle', 'square', 'triangle'];
const CYAN_GEMS: GemShape[] = ['diamond', 'star', 'hexagon'];
...
export default class Match3GameScene extends Phaser.Scene {
  ...
  startGameplay() {
    ...
    // Score bar under HUD
    this.scoreBarBg = this.add.rectangle(fx + fw / 2, fy + 28, fw - 20, 6, COLORS.GRAY, 0.15);
    this.scoreBarFill = this.add.rectangle(fx + 11, fy + 28, 0, 6, COLORS.WHITE, 0.3).setOrigin(0, 0.5);
  ...
  createGemObject(row: number, col: number, gem: GemData) {
    ...
    const cellBg = this.add.rectangle(pos.x, pos.y, CELL_SIZE - 2, CELL_SIZE - 2, COLORS.GRAY, 0.08);
    ...
    // Hit area
    const hitArea = this.add.rectangle(pos.x, pos.y, CELL_SIZE - 2, CELL_SIZE - 2, 0x000000, 0)
      .setInteractive({ useHandCursor: true });
  ...
  drawGemShape(graphics: Phaser.GameObjects.Graphics, x: number, y: number, shape: GemShape, color: number, alpha: number) {
    graphics.clear();
    ...
    switch (shape) {
      ...
      case 'triangle': {
        ...
        graphics.strokeTriangle(pts[0].x, pts[0].y, pts[1].x, pts[1].y, pts[2].x, pts[2].y);
        break;
      }
      case 'diamond': {
        const dPts = [
        ...
        for (let i = 1; i < dPts.length; i++) graphics.lineTo(dPts[i].x, dPts[i].y);
        graphics.closePath();
        graphics.strokePath();
        break;
      }
      case 'star': {
        ...
        for (let i = 0; i < 10; i++) {
          ...
          starPts.push({ x: x + Math.cos(angle) * r, y: y + Math.sin(angle) * r });
        }
        graphics.fillPoints(starPts, true);
        graphics.beginPath();
        graphics.moveTo(starPts[0].x, starPts[0].y);
        for (let i = 1; i < starPts.length; i++) graphics.lineTo(starPts[i].x, starPts[i].y);
        graphics.closePath();
        graphics.strokePath();
        break;
      }
      case 'hexagon': {
        ...
        for (let i = 0; i < 6; i++) {
          ...
          hexPts.push({ x: x + Math.cos(angle) * s, y: y + Math.sin(angle) * s });
        }
        graphics.fillPoints(hexPts, true);
        graphics.beginPath();
        graphics.moveTo(hexPts[0].x, hexPts[0].y);
        for (let i = 1; i < hexPts.length; i++) graphics.lineTo(hexPts[i].x, hexPts[i].y);
        graphics.closePath();
        graphics.strokePath();
        break;
      }
  ...
  findAllMatches(): CellPos[] {
    ...
    for (let r = 0; r < this.gridSize; r++) {
      for (let c = 0; c <= this.gridSize - 3; c++) {
        ...
        if (g0 && g1 && g2 && g0.typeIndex === g1.typeIndex && g1.typeIndex === g2.typeIndex) {
          // Extend run
          let end = c + 2;
          while (end + 1 < this.gridSize && this.grid[r][end + 1] && this.grid[r][end + 1].typeIndex === g0.typeIndex) {
            end++;
          }
          for (let k = c; k <= end; k++) {
            matched.add(`${r},${k}`);
    ...
    for (let c = 0; c < this.gridSize; c++) {
      for (let r = 0; r <= this.gridSize - 3; r++) {
        ...
        if (g0 && g1 && g2 && g0.typeIndex === g1.typeIndex && g1.typeIndex === g2.typeIndex) {
          let end = r + 2;
          while (end + 1 < this.gridSize && this.grid[end + 1][c] && this.grid[end + 1][c].typeIndex === g0.typeIndex) {
            end++;
          }
          for (let k = r; k <= end; k++) {
            matched.add(`${k},${c}`);
  ...
  async resolveMatches(matches: CellPos[]) {
    ...
    while (currentMatches.length > 0) {
      ...
      for (const cell of currentMatches) {
        ...
        GameVFX.flash(this, pos.x, pos.y, CELL_SIZE - 4, CELL_SIZE - 4, POP_DURATION);
      }

      // Score popup for chain
      if (chainMultiplier > 1) {
        const midCell = currentMatches[Math.floor(currentMatches.length / 2)];
        const midPos = this.getCellCenter(midCell.row, midCell.col);
        GameVFX.scorePopup(this, midPos.x, midPos.y, `x${chainMultiplier}`, '#FFFFFF');
      }
  ...
  advanceLevel() {
    ...
    const levelText = this.add.text(cx, cy, `${t('game.score')} ${this.level}!`, {

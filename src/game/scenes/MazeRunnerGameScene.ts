...
function generateMaze(cols, rows) {
  ...
  while (stack.length > 0) {
    ...
    const neighbors = getUnvisitedNeighbors(grid, current.r, current.c, rows, cols);
...
export default class MazeRunnerGameScene extends Phaser.Scene {
  ...
  startGameplay() {
    ...
    this.coinHudText = this.add.text(fx + fw - 8, fy + 28, '', {
      fontSize: '13px', color: COLORS.GRAY_HEX,
  ...
  buildMaze() {
    ...
    for (let r = 0; r < this.rRows; r++) {
      for (let c = 0; c < this.rCols; c++) {
        if (this.renderGrid[r][c] === 1) {
          ...
          this.mazeGraphics.fillRoundedRect(px + 0.5, py + 0.5, this.cellW - 1, this.cellH - 1, 2);
        }
      }
    }
    ...
    const coinPositions = placeCoins(
      this.renderGrid, this.rCols, this.rRows,
      this.coinCount + (this.level - 1) * 2, // more coins each level
      this.startRC, this.exitRC
    );
    this.coinObjects = [];
    ...
    for (const pos of coinPositions) {
      ...
      const obj = GameVisuals.glowCircle(this, cx, cy, coinRadius, this.ballColor, this.ballAlpha);
      GameVisuals.pulse(this, obj, 0.85, 1.15, 700);
      this.coinObjects.push({ obj, col: pos.c, row: pos.r, active: true });
    }
    ...
    this.exitGlow = this.add.circle(exitX, exitY, exitSize * 1.2, this.ballColor, this.ballAlpha * 0.15);
    ...
    this.playerObj = this.add.circle(startX, startY, playerRadius, this.platformColor, this.platformAlpha);
  ...
  isWalkable(col, row) {
    if (row < 0 || row >= this.rRows || col < 0 || col >= this.rCols) return false;
  ...
  update(time, delta) {
    ...
    if (this.coinHudText) {
      this.coinHudText.setText(`${this.levelCoinsCollected}/${this.coinsTotal}`);
    }
  }
  ...
  updatePlayer(dt) {
    if (!this.playerMoving) {
      ...
      const tryDirs = this.playerNextDir !== 'none'
  ...
  checkExitReached() {
    if (this.playerCol === this.exitRC.c && this.playerRow === this.exitRC.r) {
      this.totalExits++;
  ...
  onLevelClear() {
    ...
    const progressIdx = Math.min(currentIdx + Math.floor((this.level - 1) / 2), sizeKeys.length - 1);
  ...
  endGame(won) {
    ...
    const totalTrials = this.totalExits + this.coinsCollected + this.totalBacktracks;

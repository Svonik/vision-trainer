...
export default class MemoryTilesGameScene extends Phaser.Scene {
    ...
    startGameplay() {
        ...
        this.level = 1;
        const gridConfig =
            GRID_CONFIG[this.settings.speed] || GRID_CONFIG.normal;
        ...
        this.pairsMatched = 0;
        this.moves = 0;
        this.movesText = GameVisuals.scoreText(
            this,
            fx + 10,
            fy + 28,
            'Ходы: 0',
    ...
    renderGrid() {
        ...
        const startX =
            this.field.x + (this.field.w - gridW) / 2 + TILE_SIZE / 2;
        const startY =
            this.field.y + (this.field.h - gridH) / 2 + TILE_SIZE / 2 + 15;
    ...
    createTileObject(tx, ty, index) {
        ...
        const back = this.add
            .rectangle(tx, ty, TILE_SIZE - 4, TILE_SIZE - 4, COLORS.GRAY, 0.3)
        ...
        const hitArea = this.add
            .rectangle(tx, ty, TILE_SIZE - 4, TILE_SIZE - 4, 0x000000, 0)
            .setInteractive({ useHandCursor: true });
        ...
        hitArea.on('pointerover', () => {
            if (
                !this.tileData[index].flipped &&
                !this.tileData[index].matched
            ) {
    ...
    endGame(won) {
        ...
        const result = {
            ...
            hit_rate:
                this.totalPairs > 0
                    ? Math.round((this.pairsMatched / this.totalPairs) * 100) /
                      100
                    : 0,
            moves: this.moves,

...
export default class RunnerGameScene extends Phaser.Scene {
  ...
  startGameplay() {
    ...
    this.runnerColor = isLeftPlatform ? eyeColors.leftColor : eyeColors.rightColor;
    this.obstacleColor = isLeftPlatform ? eyeColors.rightColor : eyeColors.leftColor;
    this.runnerAlpha = (isLeftPlatform ? this.settings.contrastLeft : this.settings.contrastRight) / 100;
    this.obstacleAlpha = (isLeftPlatform ? this.settings.contrastRight : this.settings.contrastLeft) / 100;
    ...
    this.add.rectangle(fx + fw / 2, this.groundY + GROUND_THICKNESS / 2, fw, GROUND_THICKNESS, COLORS.GRAY, 0.4);
    ...
    this.runnerVisual = GameVisuals.glowRect(
      this, runnerX, runnerStartY, runnerSize, runnerSize,
      this.runnerColor, this.runnerAlpha, 3
    );
  ...
  update(time, delta) {
    ...
    if (this.trailTimer > 60) {
      ...
      if (isAirborne) {
        GameVFX.addTrailDot(
          this, this.runner.x - this.runner.size / 2,
          this.runner.y, this.runnerColor, 2, this.runnerAlpha * 0.5
        );
      }
    }
    ...
    for (let i = 0; i < this.obstacles.length; i++) {
      ...
      if (!updated.scored && updated.x + updated.w / 2 < this.runner.x - this.runner.size / 2) {
    ...
    for (let ri = toRemove.length - 1; ri >= 0; ri--) {
      ...
      if (this.obstacleGraphics[idx]) this.obstacleGraphics[idx].destroy();
      this.obstacles.splice(idx, 1);
      this.obstacleGraphics.splice(idx, 1);
    }

    // Distance-based score (increases with time)
    const distanceScore = Math.floor(this.scrollSpeed * (this.safetyTimer?.getElapsedMs() ?? 0) / 5000);
    this.score = Math.max(this.score, distanceScore);

    // HUD update
    if (this.safetyTimer && this.hud) {
      GameVisuals.updateHUD(this.hud, this.level, this.safetyTimer.getElapsedMs(), `${this.score}m`);
    }
  }
  ...
  jump() {
    ...
    if (!isFirstJump) {
      GameVFX.circleFlash(
        this, this.runner.x, this.runner.y + this.runner.size / 2,
        this.runner.size * 0.6, this.runnerColor, 150
      );
    }
  }
  ...
  spawnObstacle() {
    ...
    const types = [
      ...
      { wRatio: 0.03, hRatio: 0.10 },  // medium
    ...
    const gfx = GameVisuals.glowRect(
      this, obsX, obsY, obsW, obsH,
      this.obstacleColor, this.obstacleAlpha, 2
    );
  ...
  triggerGameOver() {
    ...
    GameVFX.particleBurst(this, this.runner.x, this.runner.y, this.runnerColor, 10);
  ...
  endGame() {
    ...
    const result = {
      ...
      hit_rate: this.totalSpawned > 0
        ? Math.round((this.obstaclesPassed / this.totalSpawned) * 100) / 100
        : 0,

...
export default class ShootingGalleryGameScene extends Phaser.Scene {
  ...
  startGameplay() {
    ...
    this.targetColor = isTargetLeft ? eyeColors.leftColor : eyeColors.rightColor;
    this.crosshairColor = isTargetLeft ? eyeColors.rightColor : eyeColors.leftColor;
    this.targetAlpha = (isTargetLeft ? this.settings.contrastLeft : this.settings.contrastRight) / 100;
    this.crosshairAlpha = (isTargetLeft ? this.settings.contrastRight : this.settings.contrastLeft) / 100;
    ...
    const speedMultipliers = { slow: 0.7, normal: 1.0, fast: 1.3, pro: 1.6 };
  ...
  drawLaneFences() {
    ...
    for (const ly of this.laneYs) {
      ...
      while (cx < this.field.x + this.field.w) {
        g.moveTo(cx, ly + 25);
        g.lineTo(Math.min(cx + dashLen, this.field.x + this.field.w), ly + 25);
        cx += dashLen + gapLen;
      }
    }
    g.strokePath();
    ...
    for (const ly of this.laneYs) {
      this.add.rectangle(this.field.x + 6, ly + 25, 3, 12, COLORS.GRAY, 0.15);
      this.add.rectangle(this.field.x + this.field.w - 6, ly + 25, 3, 12, COLORS.GRAY, 0.15);
    }
  }
  ...
  spawnTarget() {
    ...
    const speed = ttype.speed * this.speedMul * levelSpeedBoost * (goingRight ? 1 : -1);

    // Scale radius down slightly at higher levels
    const levelRadiusShrink = Math.max(0.7, 1 - (this.level - 1) * 0.04);
    const radius = Math.max(6, Math.round(ttype.radius * levelRadiusShrink));

    // Create target visual (target eye color)
    const visual = GameVisuals.glowCircle(this, startX, laneY, radius, this.targetColor, this.targetAlpha);

    // Points label inside (gray, both eyes)
    const label = this.add.text(startX, laneY, `${ttype.points}`, {
  ...
  update(time, delta) {
    ...
    if (elapsed - this.lastSpawnTime >= this.spawnInterval && this.targets.length < MAX_TARGETS) {
      this.spawnTarget();
    ...
    for (let i = this.targets.length - 1; i >= 0; i--) {
      ...
      if (tgt.speed > 0 && tgt.x > this.field.x + this.field.w + tgt.radius + 10) {
        tgt.visual.destroy();
        tgt.label.destroy();
        this.targets.splice(i, 1);
      } else if (tgt.speed < 0 && tgt.x < this.field.x - tgt.radius - 10) {
        tgt.visual.destroy();
        tgt.label.destroy();
        this.targets.splice(i, 1);
      }
  ...
  resetForNextLevel() {
    ...
    this.spawnInterval = Math.max(400, SPAWN_INTERVAL_MS - (this.level - 1) * 100);
  }
  ...
  // --- Cleanup ---
  shutdown() {
    EventBus.removeListener('start-shootinggallery-game', this.startGameHandler);

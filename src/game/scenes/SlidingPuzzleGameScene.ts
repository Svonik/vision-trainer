...
export default class SlidingPuzzleGameScene extends Phaser.Scene {
    ...
    calculateTileSize() {
        ...
        const maxTileW =
            (availW - (this.gridSize - 1) * TILE_GAP) / this.gridSize;
        const maxTileH =
            (availH - (this.gridSize - 1) * TILE_GAP) / this.gridSize;
    ...
    slideTile(tileIndex, emptyIndex) {
        ...
        const emptyCol = emptyIndex % this.gridSize;
        const emptyRow = Math.floor(emptyIndex / this.gridSize);
        const { x: targetX, y: targetY } = this.tilePosition(
            emptyCol,
            emptyRow,
        );
    ...
    onPuzzleSolved() {
        ...
        const _cx = this.field.x + this.field.w / 2;
        const _cy = this.field.y + this.field.h / 2;

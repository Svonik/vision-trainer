...
export default class SnakeGameScene extends Phaser.Scene {
    ...
    update(_time, delta) {
        ...
        const touchUpJust =
            this.touchDPad?.up?.isDown && this._touchDirFired !== 'up';
        const touchDownJust =
            this.touchDPad?.down?.isDown && this._touchDirFired !== 'down';
        const touchLeftJust =
            this.touchDPad?.left?.isDown && this._touchDirFired !== 'left';
        const touchRightJust =
            this.touchDPad?.right?.isDown && this._touchDirFired !== 'right';
        ...
        if (this.foodPulseScale >= 1.2) {
            this.foodPulseScale = 1.2;
            this.foodPulseDir = -1;
        }
        if (this.foodPulseScale <= 0.8) {
            this.foodPulseScale = 0.8;
            this.foodPulseDir = 1;
        }
        this.renderFood();
    ...
    moveSnake() {
        ...
        if (
            ...
            const { x: foodPx, y: foodPy } = this.cellToPixel(
                this.food.x,
                this.food.y,
            );
            SynthSounds.score();
    ...
    renderSnake() {
        ...
        this.snake.forEach((seg, index) => {
            ...
            if (index === 0) {
                ...
                this.snakeGraphics.fillStyle(
                    this.platformColor,
                    this.platformAlpha * 0.15,
                );
                ...
                this.snakeGraphics.fillStyle(
                    this.platformColor,
                    Math.min(1, this.platformAlpha * 1.3),
                );
                this.snakeGraphics.fillRect(px - w / 2, py - h / 2, w, h);
                // Head highlight strip
                this.snakeGraphics.fillStyle(
                    this.platformColor,
                    Math.min(1, this.platformAlpha * 1.5),
                );
                this.snakeGraphics.fillRect(
                    px - w / 2 + 2,
                    py - h / 2 + 2,
    ...
    renderFood() {
        ...
        const radius =
            (Math.min(this.cellW, this.cellH) / 2 - 2) * this.foodPulseScale;

        // Outer glow layers
        this.foodGraphics.fillStyle(this.ballColor, this.ballAlpha * 0.06);
        this.foodGraphics.fillCircle(px, py, radius * 2.0);
        this.foodGraphics.fillStyle(this.ballColor, this.ballAlpha * 0.1);
        ...
        // Inner highlight
        this.foodGraphics.fillStyle(
            this.ballColor,
            Math.min(this.ballAlpha * 1.3, 1),
        );
        this.foodGraphics.fillCircle(px, py, radius * 0.4);
    }

    updateFellowEyeAlpha(alpha) {
        // Update snake (fellow eye) alpha — food (amblyopic eye) stays at 1.0
    ...
    endGame() {
        ...
        const result = {
            ...
            duration_s: Math.round(
                (this.safetyTimer ? this.safetyTimer.getElapsedMs() : 0) / 1000,
            ),
            caught: this.score,

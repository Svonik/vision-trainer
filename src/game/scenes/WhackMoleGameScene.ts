...
export default class WhackMoleGameScene extends Phaser.Scene {
  ...
  startGameplay() {
    ...
    this.moleColor = isMoleLeft ? eyeColors.leftColor : eyeColors.rightColor;
    this.crosshairColor = isMoleLeft ? eyeColors.rightColor : eyeColors.leftColor;
    this.moleAlpha = (isMoleLeft ? this.settings.contrastLeft : this.settings.contrastRight) / 100;
    this.crosshairAlpha = (isMoleLeft ? this.settings.contrastRight : this.settings.contrastLeft) / 100;
    ...
    this.spawnInterval = SPAWN_INTERVAL_MS[speed] || SPAWN_INTERVAL_MS.normal;
  ...
  buildHoleGrid() {
    ...
    for (let row = 0; row < GRID_ROWS; row++) {
      for (let col = 0; col < GRID_COLS; col++) {
        ...
        holeG.fillEllipse(cx, cy + HOLE_RADIUS * 0.3, HOLE_RADIUS * 2.2, HOLE_RADIUS * 1.2);

        // Hole ring
        holeG.lineStyle(1.5, COLORS.GRAY, 0.25);
        holeG.strokeEllipse(cx, cy + HOLE_RADIUS * 0.3, HOLE_RADIUS * 2.2, HOLE_RADIUS * 1.2);
  ...
  spawnMole() {
    ...
    const glowOuter = this.add.circle(0, 0, MOLE_RADIUS * 1.5, this.moleColor, this.moleAlpha * 0.08);
    const glowMid = this.add.circle(0, 0, MOLE_RADIUS * 1.2, this.moleColor, this.moleAlpha * 0.12);
    const body = this.add.circle(0, 0, MOLE_RADIUS, this.moleColor, this.moleAlpha);
    const highlight = this.add.circle(0, -MOLE_RADIUS * 0.3, MOLE_RADIUS * 0.35, this.moleColor, Math.min(this.moleAlpha * 1.3, 1));
  ...
  hitMole(index) {
    ...
    this.hits++;
    if (this.hud) this.hud.scoreText.setText(`\u2605 ${this.hits}/${WIN_COUNT}`);
  ...
  update(time, delta) {
    ...
    if (now - this.lastSpawnTime >= this.spawnInterval && this.moles.length < this.maxMoles) {
    ...
    // HUD update
    if (this.hud) {
      GameVisuals.updateHUD(this.hud, this.level, this.safetyTimer.getElapsedMs(), `\u2605 ${this.hits}/${WIN_COUNT}`);
    }
  }
  ...
  resetForNextLevel() {
    ...
    this.spawnInterval = Math.max(400, Math.round(this.spawnInterval * 0.85));

...
export const GameVisuals = {
   ...
   */
  glowCircle: (scene: Phaser.Scene, x: number, y: number, radius: number, color: number, alpha: number = 1): Phaser.GameObjects.Container => {
    const container = scene.add.container(x, y);
    // Outer glow (3 layers, decreasing alpha)
    container.add(scene.add.circle(0, 0, radius * 2.0, color, alpha * 0.04));
    container.add(scene.add.circle(0, 0, radius * 1.5, color, alpha * 0.08));
    container.add(scene.add.circle(0, 0, radius * 1.2, color, alpha * 0.12));
    // Core body
    container.add(scene.add.circle(0, 0, radius, color, alpha));
    // Inner highlight (brighter center)
    container.add(scene.add.circle(0, 0, radius * 0.4, color, Math.min(alpha * 1.3, 1)));
    return container;
  },
   ...
   */
  glowRect: (scene: Phaser.Scene, x: number, y: number, w: number, h: number, color: number, alpha: number = 1, radius: number = 4): Phaser.GameObjects.Container => {
    const container = scene.add.container(x, y);
    // Outer glow
    const glowG = scene.add.graphics();
    glowG.fillStyle(color, alpha * 0.06);
    glowG.fillRoundedRect(-w / 2 - 6, -h / 2 - 6, w + 12, h + 12, radius + 4);
    container.add(glowG);
    // Body
   ...
   */
  drawBgGrid: (scene: Phaser.Scene, fx: number, fy: number, fw: number, fh: number, cellSize: number = 40) => {
   ...
   */
  styledBorder: (scene: Phaser.Scene, fx: number, fy: number, fw: number, fh: number) => {
    const g = scene.add.graphics();
    // Main border
    ...
    // Top-left
    g.moveTo(fx, fy + cornerLen); g.lineTo(fx, fy); g.lineTo(fx + cornerLen, fy);
    // Top-right
    g.moveTo(fx + fw - cornerLen, fy); g.lineTo(fx + fw, fy); g.lineTo(fx + fw, fy + cornerLen);
    // Bottom-left
    g.moveTo(fx, fy + fh - cornerLen); g.lineTo(fx, fy + fh); g.lineTo(fx + cornerLen, fy + fh);
    // Bottom-right
    g.moveTo(fx + fw - cornerLen, fy + fh); g.lineTo(fx + fw, fy + fh); g.lineTo(fx + fw, fy + fh - cornerLen);
    g.strokePath();
    // Corner dots
   ...
   */
  pulse: (scene: Phaser.Scene, target: Phaser.GameObjects.GameObject, min: number = 0.95, max: number = 1.05, duration: number = 800) => {
   ...
   */
  rotate: (scene: Phaser.Scene, target: Phaser.GameObjects.GameObject, speed: number = 0.5) => {
    return scene.tweens.add({
      targets: target,
      angle: 360,
   ...
   */
  scoreText: (scene: Phaser.Scene, x: number, y: number, text: string, align: number = 0): Phaser.GameObjects.Text => {

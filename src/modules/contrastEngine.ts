...
export function createContrastState(initialFellowContrast: number): ContrastState {
...
export function getContrastProgress(state: ContrastState, config: ContrastConfig): number {
  if (config.ceiling <= config.floor) return 0;
  return (state.fellowEyeContrast - config.floor) / (config.ceiling - config.floor);
}
...
export function recordTrial(
  ...
  const trimmed = newWindow.length > config.windowSize
  ...
  if (trimmed.length >= config.windowSize) {
    ...
    if (accuracy > config.stepUpThreshold && newContrast < config.ceiling) {
      newContrast = Math.min(newContrast + config.stepSize, config.ceiling);
      stepDirection = 'up';
    } else if (accuracy < config.stepDownThreshold && newContrast > config.floor) {

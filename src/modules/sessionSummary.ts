...
function computeStars(hitRate: number): 1 | 2 | 3 {
    if (hitRate >= 0.8) return 3;
    if (hitRate >= 0.6) return 2;
    return 1;
}
...
export function computeSessionSummary(
    ...
    const fellowEnd =
        currentResult.fellow_contrast_end ?? CLINICAL_CONTRAST.FELLOW_INITIAL;
    const range =
        CLINICAL_CONTRAST.FELLOW_CEILING - CLINICAL_CONTRAST.FELLOW_FLOOR;
    const contrastProgress =
        range > 0
            ? ((fellowEnd - CLINICAL_CONTRAST.FELLOW_FLOOR) / range) * 100
            : 0;

    const totalSeconds = allIncludingCurrent.reduce(
    ...
    const sameGameSessions = allSessions.filter(
        (s) => s.game === currentResult.game,
    );
    const bestPreviousAccuracy =
        sameGameSessions.length > 0
            ? Math.max(...sameGameSessions.map((s) => s.hit_rate))
            : 0;
    const isNewRecord =
        currentResult.hit_rate > bestPreviousAccuracy &&
        sameGameSessions.length > 0;

...
export function getConsecutiveAdverseCount(sessions: readonly SessionResult[]): number {
  let count = 0;
  for (let i = sessions.length - 1; i >= 0; i--) {
    const wellness = (sessions[i] as any).wellness as WellnessCheck | null | undefined;
    if (!wellness) break;
    if (hasAdverseSymptoms(wellness)) { count++; } else { break; }
  }
  return count;
}

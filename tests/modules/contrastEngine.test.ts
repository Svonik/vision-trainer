...
describe('contrastEngine', () => {
    ...
    describe('createContrastState', () => {
        ...
        it('clamps initial contrast to floor', () => {
            ...
            expect(state.fellowEyeContrast).toBe(
    ...
    describe('recordTrial', () => {
        ...
        it('paces steps: 20 misses in a row after the window is full cause at most 1 step down', () => {
            ...
            // Fill the rolling window with hits — establishes a baseline contrast
            // (this fill itself may trigger a single step, which is expected).
            for (let i = 0; i < config.windowSize; i++) {
                state = recordTrial(state, config, true);
            }
            const contrastAfterFill = state.fellowEyeContrast;

            // A losing streak exactly one window long must not be able to ratchet
            // the contrast down on every single trial — pacing allows at most one
            // step per ROLLING_WINDOW_SIZE new trials since the last step.
            for (let i = 0; i < config.windowSize; i++) {
                state = recordTrial(state, config, false);
            }

            const drop = contrastAfterFill - state.fellowEyeContrast;
            expect(drop).toBeLessThanOrEqual(config.stepSize);
        });
    });
